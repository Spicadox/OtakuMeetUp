    ...
    User.find({ email: req.body.email, password: req.body.password })
        .then((queryLogin) => {
            ...
            if (queryLogin.length > 0) {
                UserProfile.find({ user: queryLogin })
                    .then((userProfile) => {
...
router.post(['/savedConnections'], async function (req, res) {
    ...
    if (req.session.previousPath == "/connection") {
        ...
        let newUserConnection = new UserConnection({ connection: newConnection, rsvp: req.body.rsvp });
        console.log("ID");
        console.log(req.session.userProfile)
        console.log(req.session.userProfile[0]._id)
        console.log(newUserProfile._id)
        await UserProfile.findOne({ _id: newUserProfile._id }).exec(function (err, result) {
            console.log(result);
            if (result == null) {
                UserProfile.findOneAndUpdate({ _id: newUserProfile._id }, { userConnections: newUserConnection }).exec(function (err, result) {
