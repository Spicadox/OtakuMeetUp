...
router.post(['/savedConnections'], function (req, res) {
    ...
    if (req.session.previousPath == "/connection") {
        ...
        let newUserProfile = new UserProfile(new User({fName: req.session.user.fName, lName: req.session.user.lName, email: req.session.user.email, fName: req.session.user.fName,}), new UserConnection([{ userConnection: new Connection() }]));
