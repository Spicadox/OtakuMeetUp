...
router.post(['/savedConnections'], async function (req, res) {
    ...
    if (req.session.previousPath == "/connection") {
        ...
        try {
            let addedConnection = await newUserProfile.addConnection(newUserConnection);
        } catch (err) {
            console.log(err);
        }
        ...
        try {
            let allConnections = await newUserProfile.getConnections();
        } catch (err) {
            console.log(err)
        }
        ...
        console.log("END")
        req.session.userProfileConnections = allConnections;
        res.render("savedConnections", { userProfile: req.session.userProfileConnections, req: req });
    }
