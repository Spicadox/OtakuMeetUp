...
router.post(['/savedConnections'], async function (req, res) {
    ...
    if (req.session.previousPath == "/connection") {
        ...
        let newUserConnection = async new UserConnection({ _id: Mongoose.Types.ObjectId(), connection: newConnection, rsvp: req.body.rsvp });
        newUserConnection.save().then(() => console.log('saved new connection'));
