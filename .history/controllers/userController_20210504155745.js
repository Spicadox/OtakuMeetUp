...
router.post(['/savedConnections'], function (req, res) {
    ...
    if (req.session.previousPath == "/connection") {
        ...
        let newUserProfile = new UserProfile(new User({
            fName: req.session.user.fName,
            lName: req.session.user.lName,
            email: req.session.user.email,
            password: req.session.user.passowrd,
        }), new UserConnection([{ userConnection: new Connection() }]));
        let parsedConnection = JSON.parse(req.body.connection);
        let newConnection = new Connection(parsedConnection._id, parsedConnection.title, parsedConnection._category, parsedConnection._details, parsedConnection._dateTime, parsedConnection._location);
