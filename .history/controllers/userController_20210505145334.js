...
router.post(['/login', '/login.ejs'], async function (req, res) {
    let queryUser = await User.find({ email: req.body.email, password: req.body.password })

    // temporary include length of > 0 instead of >= 0
    // console.log(queryLogin);
    if (queryUser.length > 0) {
        let queryUserProfile = await UserProfile.findOne({ user: queryLogin })
    ...
    try {
        console.log("GETTING")
        req.session.userProfileConnections = await queryUserProfile.getConnections(queryUserProfile._id);
    } catch (err) {
        console.log(err)
    }
...
router.post(['/savedConnections'], async function (req, res) {
    ...
    if (req.session.previousPath == "/connection") {
        ...
        let newUserProfile = await UserProfile.findOne({ _id: req.session.userProfile._id });
        // if (userProfile.length == 0) {
        ...
        try {
            req.session.userProfileConnections = await newUserProfile.getConnections(newUserProfile._id);
        } catch (err) {
            console.log(err)
        }
