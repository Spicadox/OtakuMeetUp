...
//
for (let i = 0; i < hardcodedConnections.length; i++) {
    Connection.find(hardcodedConnections[i]).then((result) => {
        // if connections database doesn't contain the hardcoded connections already
        if (result.length === 0) {
            new Connection(connhardcodedConnectionsections[i]).save();
            console.log("saved")
        }
    });
}
