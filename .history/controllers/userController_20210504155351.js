...
router.post(['/savedConnections'], function (req, res) {
    ...
    if (req.session.previousPath == "/connection") {
        ...
        let newUserProfile = new UserProfile(new User({fName: req.session.user.fName, lName: req.session.user.lName, email: req.session.user.email, password: req.session.user.passowrd,}), new UserConnection([{ userConnection: new Connection() }]));
