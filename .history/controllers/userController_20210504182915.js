...
router.post(['/savedConnections'], async function (req, res) {
    ...
    if (req.session.previousPath == "/connection") {
        ...
        (await UserProfile.findOne({_id: newUserProfile._id})).populated({path: 'userConnections', model:'userConnection'}).exec(function(err, result) {
            
        })
        if (newUserProfile.userConnections.length == 0) {
            newUserProfile.userConnections = [newUserConnection];
        }
