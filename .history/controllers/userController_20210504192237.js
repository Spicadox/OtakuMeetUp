...
router.post(['/savedConnections'], async function (req, res) {
    ...
    if (req.session.previousPath == "/connection") {
        ...
        let newUserConnection = new UserConnection({ _id: Mongoose.Types.ObjectId(), connection: newConnection, rsvp: req.body.rsvp }).save();
        ...
        await UserProfile.findOne({ _id: newUserProfile._id }).populate({ path: 'userConnections', model: 'userConnection' }).exec(function (err, result) {
            ...
            if (result.userConnections.length == 0) {
                UserProfile.findOneAndUpdate({ _id: newUserProfile._id }, { userConnections: newUserConnection }).exec(function (err, result) {
                    ...
                    console.log(result)
                    console.log(result.userConnections)
                })
            }
        })
