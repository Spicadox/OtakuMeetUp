        ...
        UserProfile.findOne({ _id: this._id }, 'userConnections', { new: true })
            .populate({ path: 'userConnections', model: 'userConnection', populate: { path: 'connection', model: 'connection' } }).exec(function (err, updatedUserProfile) {
                ...
                for (let i = 0; i < updatedUserProfile.length; i++) {
                    if (updatedUserProfile.userConnections.connection.title == userConnection.connection.title && updatedUserProfile[i].userConnections.connection.category == userConnection.connection.category) {
