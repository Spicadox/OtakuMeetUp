...
userProfileSchema.methods.addConnection = function (newUserProfile, userConnection) {
    ...
    return new Promise(function (resolve, reject,) {
        ...
        UserProfile.findOneAndUpdate({ _id: this._id }, { userConnections: userConnection })
            .populate({ path: 'userConnections', model: 'userConnection' }).exec(function (err, updatedUserProfile) {
                console.log("FIND AND UPDATE");
                console.log(updatedUserProfile)
...
userProfileSchema.methods.getConnections = async function () {
    return new Promise(function (resolve, reject) {
        UserProfile.find({ _id: this._id }, 'userConnections').populate({ path: 'userConnections', model: 'userConnection', populate: { path: 'connection', model: 'connection' } }).exec(function (err, populatedUserConnections) {
            console.log("POPULATEDUSERPROFILE");
            console.log(populatedUserConnections);
            if (populatedUserConnections != null) {
                return resolve(populatedUserConnections);
            }
            else {
