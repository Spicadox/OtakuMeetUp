...
userProfileSchema.methods.getConnections = async function () {
    return new Promise(function (resolve, reject) {
        UserProfile.find({ _id: this._id }, ).populate({ path: 'userConnections', model: 'userConnection', populate: { path: 'connection', model: 'connection' } }).exec(function (err, populatedUserProfile) {
