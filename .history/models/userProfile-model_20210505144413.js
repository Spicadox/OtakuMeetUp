...
userProfileSchema.methods.addConnection = function (newUserProfile, userConnection) {
    ...
    return new Promise(function (resolve, reject,) {
        ...
        const filter = { _id: this._id, 'userConnections.connection': userConnection };
        const update = { $set: { 'userConnections.rsvp': userConnection.rsvp }, $push: { 'userConnections': userConnection } };
        UserProfile.findOne({ _id: newUserProfile._id }, 'userConnections', { new: true })
...
userProfileSchema.methods.getConnections = async function (profileId) {
    return new Promise(function (resolve, reject) {
        UserProfile.findOne({ _id: profileId }).populate({ path: 'userConnections', model: 'userConnection', populate: { path: 'connection', model: 'connection' } }).then(function (err, populatedUserProfile) {
