...
userProfileSchema.methods.addConnection = async function (userConnection) {

    console.log("RESULT");
    await UserProfile.findOneAndUpdate({ _id: this._id }, { userConnections: userConnection }, { upsert: true })
...
userProfileSchema.methods.getConnections = async function () {
    ...
    await UserProfile.findOne({ _id: this._id }).populate({ path: 'userConnections', model: 'userConnection' }).exec(function (err, populatedUserProfile) {
