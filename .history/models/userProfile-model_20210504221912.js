...
userProfileSchema.methods.addConnection = function (userConnection) {
    ...
    return Promise(function (resolve, reject) {
        UserProfile.findOneAndUpdate({ _id: this._id }, { userConnections: userConnection }, { upsert: true })
            .populate({ path: 'userConnections', model: 'userConnection' }).exec(function (err, updatedUserProfile) {
                ...
                if (updatedUserProfile != null) {
                    ...
                    return new Promise((reject) => reject("FAILURE"));
                }
            })
    })
...
userProfileSchema.methods.getConnections = async function () {
    return Promise(function (resolve, reject) {
        UserProfile.findOne({ _id: this._id }).populate({ path: 'userConnections', model: 'userConnection' }).exec(function (err, populatedUserProfile) {
            console.log("POPULATEDUSERPROFILE");
            console.log(populatedUserProfile);
            if (populatedUserProfile != null) {
                return new Promise((resolve) => resolve(populatedUserProfile.userConnections));
            }
            else {
