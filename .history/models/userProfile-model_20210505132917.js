        ...
        UserProfile.findOne({ _id: this._id },'userConnections', { new: true })
            .populate({ path: 'userConnections', model: 'userConnection' }).exec(function (err, updatedUserProfile) {
                ...
                for(let i=0; i < updatedUserProfile.length; i++){
                    if(updatedUserProfile[i].connection.title == userConnection.connection.title && updatedUserProfile[i].connection.category == userConnection.connectioncategory) {
                        if(updatedUserProfile[i].rsvp != userConnection.)
                    }
                }
