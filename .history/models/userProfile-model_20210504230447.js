...
userProfileSchema.methods.addConnection = function (newUserProfile, userConnection) {
    ...
    return new Promise(function (resolve, reject,) {
        ...
        UserProfile.findOneAndUpdate({ _id: this._id }, { $set: { userConnections: userConnection } })
        .populate({ path: 'userConnections', model: 'userConnection' }).exec(function (err, updatedUserProfile) {
            console.log("FIND AND UPDATE");
            console.log(updatedUserProfile)

            if(updatedUserProfile == null) {
                UserProfile.findOne({ _id: this._id }).populate({ path: 'userConnections', model: 'userConnection' }).exec(function (err, updatedUserProfile) {
                }
            }
