...
userProfileSchema.methods.addConnection = function (newUserProfile, userConnection) {
    ...
    return new Promise(function (resolve, reject,) {
        ...
        UserProfile.find({_id: this._id}, 'userConnections').populate({ path: 'userConnections', model: 'userConnection' }).exec(function (userConnections) {
            userConnections.findOneAndUpdate({ 'userConnections.connection': userConnection }, { $push: { 'userConnections': userConnection } }, { new: true })
            .populate({ path: 'userConnections', model: 'userConnection' }).exec(function(err, updated))
        });

        const filter = { _id: this._id, 'userConnections.connection': userConnection };
        const update = { 'userConnections.$.rsvp': userConnection.rsvp, $push: { 'userConnections': userConnection } };
        UserProfile.findOneAndUpdate({ _id: this._id }, { $push: { 'userConnections': userConnection } }, { new: true })
            .populate({ path: 'userConnections', model: 'userConnection' }).exec(function (err, updatedUserProfile) {
                console.log("FIND AND UPDATE");
                console.log(updatedUserProfile)
