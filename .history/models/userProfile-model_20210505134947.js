        ...
        UserProfile.findOne({ _id: this._id }, 'userConnections', { new: true })
            .populate({ path: 'userConnections', model: 'userConnection', populate: { path: 'connection', model: 'connection' } }).exec(function (err, updatedUserProfile) {
                ...
                for (let i = 0; i < updatedUserProfile.userConnections.length; i++) {
                    ...
                    if (updatedUserProfile.userConnections[i].connection.title == userConnection.connection.title && updatedUserProfile.userConnections[i].connection.category == userConnection.connection.category && i == updatedUserProfile.userConnections.length - 1) {
                        if (updatedUserProfile.userConnections[i].rsvp != userConnection.connection.rsvp) {
                            updatedUserProfile[i].userConnections.rsvp = userConnection.connection.rsvp;
